...
class EulerAnglesZXY {
  ...
  */
  ...
  Eigen::Quaternion <T> ToQuaternion() const {
    ...
    T sr = std::sin(r);//0
    T sp = std::sin(p);//0
    T sy = std::sin(y);//sin(y)

    T cr = std::cos(r);//1
    T cp = std::cos(p);//1
    T cy = std::cos(y);//cos(y)

    T qw = cr * cp * cy - sr * sp * sy;//cos(y)
    T qx = cr * sp * cy - sr * cp * sy;//0
    T qy = cr * sp * sy + sr * cp * cy;//0
    T qz = cr * cp * sy + sr * sp * cy;//sin(y);
...
void EulerAnglesZXYTest_SingleConstruct();
void EulerAnglesZXYTest_FullConstructDouble();
void EulerAnglesZXYTest_FullConstructFloat();
void EulerAnglesZXYTest_IsValid();
