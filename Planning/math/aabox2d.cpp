...
void AABox2d::GetAllCorners(std::vector<Vec2d> *const corners) const {
  ...
  corners->reserve(4);
  Vec2d vec2d1(center_.x() + half_length_, center_.y() - half_width_);
  corners->emplace_back(vec2d1);
  Vec2d vec2d2(center_.x() + half_length_, center_.y() + half_width_);
  corners->emplace_back(vec2d2);
  Vec2d vec2d3(center_.x() - half_length_, center_.y() + half_width_);
  corners->emplace_back(vec2d3);
  Vec2d vec2d4(center_.x() - half_length_, center_.y() - half_width_);
  corners->emplace_back(vec2d4);
...
bool AABox2d::HasOverlap(const AABox2d &box) const {
  std::cout<<"box.center = "<< box.center_x() <<","
                            << box.center_y()<<std::endl;
  std::cout<<"center_ = "<< center_.x()<<","
                         << center_.y()<<std::endl;
...
void AABox2d::Shift(const Vec2d &shift_vec) {
  center_.set_x(center_.x()+shift_vec.x());
  center_.set_y(center_.y()+shift_vec.y());}
