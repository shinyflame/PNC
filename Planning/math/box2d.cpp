...
Box2d::Box2d(const Vec2d &center, const double heading, const double length,
    ...
    : center_(center),
      ...
      heading_(heading + M_PI_2),//
      cos_heading_(cos(heading + M_PI_2)),
      sin_heading_(sin(heading + M_PI_2)) {
...
void Box2d::ExtendInLength(double forwards,double rearwards){

    const double fx = cos_heading_ * forwards;
    const double fy = sin_heading_ * forwards;
    const double rx = cos_heading_ * rearwards;
    const double ry = sin_heading_ * rearwards;
    const double dx =  sin_heading_ * half_width_;
    const double dy = -cos_heading_ * half_width_;
    corners_.clear();
    Vec2d corner1(center_.x() + fx + dx, center_.y() + fy + dy);
    corners_.emplace_back(corner1);
    Vec2d corner2(center_.x() + fx - dx, center_.y() + fy - dy);
    corners_.emplace_back(corner2);
    Vec2d corner3(center_.x() - rx - dx, center_.y() - ry - dy);
    corners_.emplace_back(corner3);
    Vec2d corner4(center_.x() - rx + dx, center_.y() - ry + dy);
    corners_.emplace_back(corner4);
...
std::string Box2d::DebugString() const {
  return "box2d ( center = " + center_.DebugString()+
         "  heading = " + std::to_string(heading_)+
