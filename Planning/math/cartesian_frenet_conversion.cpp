...
void CartesianFrenetConverter::cartesian_to_frenet(
  ...
  const double cos_theta_r = std::cos(rtheta + M_PI/2.0);
  const double sin_theta_r = std::sin(rtheta + M_PI/2.0);
...
void CartesianFrenetConverter::frenet_to_cartesian(
  ...
  *ptr_theta = NormalizeAngle(delta_theta + rtheta + M_PI/2.0);
