...
void Polygon2d::BuildFromPoints() {
 ...
 // CHECK_GT(area_, kMathEpsilon);
  ...
  for (int i = 0; i < num_points_; ++i) {
    LineSegment2d line_segment(points_[i], points_[Next(i)]);
    line_segments_.emplace_back(line_segment);
  }
 ...
 // CHECK_GE(points_.size(), 3);
  ...
  for (size_t i = 0; i + 1 < projections.size(); ++i) {
    ...
    if (overlaps.empty() ||
       ...
       std::pair<double, double>  overlaps_one(start_proj, end_proj);
      overlaps.emplace_back(overlaps_one);
    } else {
  ...
  for (const auto &overlap : overlaps) {
      LineSegment2d  overlap_line_segment(
    ...
    overlap_line_segments.emplace_back(overlap_line_segment);
  }
...
std::string Polygon2d::point_to_string(){

//  std::string point;
//  for(int i=0; i < points_.size();i++){

//      point = point + "(" + std::to_string( points_.at(i).x() ) + " , "+
//                          + std::to_string( points_.at(i).y() ) + ") ";
//    }
