    ...
    const std::vector<std::pair<STPoint, STPoint>>& point_pairs)
{
  ...
  if(!IsValid(point_pairs)) {
      cout<< "The input point_pairs are NOT valid"<<endl;  }
...
std::string StBoundary::TypeName(BoundaryType type) {
  ...
  cout << "Unkown boundary type " << static_cast<int>(type)
        << ", treated as UNKNOWN";
  return "UNKNOWN";
}
...
bool StBoundary::IsValid(
  ...
  if (point_pairs.size() < 2) {
    cout << "point_pairs.size() must > 2. current point_pairs.size() = "
           << point_pairs.size();
    return false;
  }
  ...
  for (size_t i = 0; i < point_pairs.size(); ++i) {
    ...
    if (curr_upper.s() < curr_lower.s()) {
      cout << "s is not increasing";
      return false;
    }

    if (std::fabs(curr_lower.t() - curr_upper.t()) > kStBoundaryEpsilon) {
      cout << "t diff is larger in each STPoint pair";
      return false;
    }
    ...
    if (i + 1 != point_pairs.size()) {
      ...
      if (std::fmax(curr_lower.t(), curr_upper.t()) + kMinDeltaT >=
        ...
        cout << "t is not increasing";
        cout << " curr_lower: " << curr_lower.DebugString();
        cout << " curr_upper: " << curr_upper.DebugString();
        cout << " next_lower: " << next_lower.DebugString();
        cout << " next_upper: " << next_upper.DebugString();
        return false;
      }
    }
  }
...
StBoundary StBoundary::ExpandByT(const double t) const {
  if (lower_points_.empty()) {
    cout << "The current st_boundary has NO points.";
    return StBoundary();
  }
...
const int& StBoundary::id() const { return id_; }

void StBoundary::SetId(const int& id) { id_ = id; }
  ...
  *s_lower = 0.0;
  ...
  if (boundary_type_ == BoundaryType::STOP ||
    ...
    *s_lower = std::fmax(*s_lower, upper_cross_s);
    ...
    cout << "boundary_type is not supported. boundary_type: "
