...
class IndexedList {
 public:
  ...
  T* Find(const I id) {
    ...
    if(object_dict_.find(id)==object_dict_.end())
      return NULL;
     else
      return &object_dict_.find(id)->second;
  }
...
class ThreadSafeIndexedList : public IndexedList<I, T> {
 public:
  T* Add(const I id, const T& object) {
    ...
    std::unique_lock<std::mutex> writer_lock(mutex_);
    return IndexedList<I, T>::Add(id, object);
  }

  T* Find(const I id) {
    //boost::shared_lock<boost::shared_mutex> reader_lock(mutex_);
    std::unique_lock<std::mutex> reader_lock(mutex_);
